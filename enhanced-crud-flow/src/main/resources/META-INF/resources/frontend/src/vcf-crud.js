...
import './vcf-dialog-layout.js';
import './vcf-crud-grid.js';
import './vcf-crud-form.js';
import { html } from '@polymer/polymer/lib/utils/html-tag.js';
import { FlattenedNodesObserver } from '@polymer/polymer/lib/utils/flattened-nodes-observer.js';
import { afterNextRender } from '@polymer/polymer/lib/utils/render-status.js';
/**
 * `<vcf-crud>` is a Web Component for [CRUD](https://en.wikipedia.org/wiki/Create,_read,_update_and_delete) operations.
 *
 * ### Quick Start
 *
 * Assign an array to the [`items`](#/elements/vcf-crud#property-items) property.
 *
 * A grid and an editor will be automatically generated and configured based on the data structure provided.
 *
 * #### Example:
 * ```html
 * <vcf-crud items='[{"name": "John", "surname": "Lennon", "role": "singer"},
 *                      {"name": "Ringo", "surname": "Starr", "role": "drums"}]'></vcf-crud>
 * ```
 *
 * ### Data Provider Function
 *
 * Otherwise, you can provide a [`dataProvider`](#/elements/vcf-crud#property-dataProvider) function.
 * #### Example:
 * ```html
 * <vcf-crud></vcf-crud>
 * ```
 * ```js
 * const crud = document.querySelector('vcf-crud');
 ...
 * - [`<vcf-crud-grid>`](#/elements/vcf-crud-grid)
 * - [`<vcf-crud-edit-column>`](#/elements/vcf-crud-edit-column)
 * - [`<vcf-crud-form>`](#/elements/vcf-crud-form)
 *
 * ### Styling
 *
...
class CrudElement extends ElementMixin(ThemableMixin(PolymerElement)) {
  static get template() {
    ...
    <div id="container">
      <div id="main">
        <slot name="grid">
          <vcf-crud-grid theme\$="[[theme]]" id="grid" include="[[include]]" exclude="[[exclude]]" no-sort="[[noSort]]" no-filter="[[noFilter]]" no-head="[[noHead]]" hide-edit-column="[[editOnClick]]"></vcf-crud-grid>
        </slot>
      ...
      <vcf-dialog-layout theme\$="[[theme]]" id="dialog" no-close-on-outside-click="[[__isDirty]]" no-close-on-esc="[[__isDirty]]" opened="{{editorOpened}}" editor-position\$="{{editorPosition}}" mobile="[[__mobile]]" theme="crud">
        <h3 slot="header">[[__computeEditorHeader(__isNew, i18n.newItem, i18n.editItem)]]</h3>
        <div id="editor">
          <slot name="form">
            <vcf-crud-form theme\$="[[theme]]" id="form" include="[[include]]" exclude="[[exclude]]"></vcf-crud-form>
          </slot>
        </div>
...
`;
  ...
  static get properties() {
    return {
      ...
      */
       ...
       *
       * When it is defined [`exclude`](#/elements/vcf-crud#property-exclude) is ignored.
       *
       ...
       *
       * When [`include`](#/elements/vcf-crud#property-include) is defined, this parameter is ignored.
       *
  ...
  __onI18Change(i18n, grid) {
    if (grid) {
      afterNextRender(grid, () => {
        Array.from(grid.querySelectorAll('vcf-crud-edit-column'))
          .forEach(e => e.ariaLabel = i18n.editLabel);
        Array.from(grid.querySelectorAll('vcf-crud-edit'))
          .forEach(e => e.setAttribute('aria-label', i18n.editLabel));
      });
    }
  }
    ...
    if (item) {
      if (!this._fields.length && this._form._configure) {
        if (this.__model) {
          ...
          console.warn('<vcf-crud> Unable to autoconfigure form because the data structure is unknown. ' +
  ...
  /** A reference to all fields inside the [`_form`](#/elements/vcf-crud#property-_form) element */
  get _fields() {
